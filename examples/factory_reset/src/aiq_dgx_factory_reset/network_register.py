...
async def network_assessment_tool(config: NetworkAssessmentToolConfig, _builder: Builder):
    ...
    async def _run_network_assessment(input_message: str) -> str:
        ...
        if not os.path.exists(script_path_on_disk):
            return f"❌ Network assessment script not found at {script_path_on_disk}"
...
print("✅ Network Assessment tool registered successfully")

# ========================
# Network Results Reader Tool
# ========================
            ...
            if explicit:
                ...
                if config.cluster_host == "localhost":
                    ...
                    if os.path.exists(symlink_path) and os.path.isdir(symlink_path):
                        ...
                        logger.info(f"Using existing node assessment symlink: {latest_dir}")
                    else:
...
print("✅ Network Results Reader tool registered successfully")

# ========================
# Network Config Extractor
# ========================
