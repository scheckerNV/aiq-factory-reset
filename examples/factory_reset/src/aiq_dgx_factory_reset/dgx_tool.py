"""
DGX Documentation Expert Tool

This module provides accurate retrieval of DGX documentation
...
class DGXToolConfig(FunctionBaseConfig, name="dgx_tool"):
    docs_path: str = Field(default="examples/factory_reset/src/aiq_dgx_factory_reset/docs/dgx_expert",
                           description="Path to DGX documentation directory")
    persist_dir: str = Field(default="examples/factory_reset/storage/dgx_index",
...
@register_function(config_type=DGXToolConfig, framework_wrappers=[LLMFrameworkEnum.LANGCHAIN])
async def dgx_tool(config: DGXToolConfig, builder: Builder):
    """
    Search DGX documentation using accurate RAG retrieval
    """

    async def _search_dgx_docs(query: str) -> str:
        """Search DGX documentation with high-accuracy retrieval"""
        docs_path = config.docs_path
        persist_dir = config.persist_dir

        if not os.path.exists(docs_path):
            return f"❌ DGX documentation not found at {docs_path}"
        ...
        try:
            ...
            logger.info("Processing DGX documentation from %s", docs_path)

            # Check for existing index
            docstore_path = os.path.join(persist_dir, "docstore.json")
            if os.path.exists(docstore_path):
                logger.info("Loading existing DGX index...")
                storage_context = StorageContext.from_defaults(persist_dir=persist_dir)
                index = load_index_from_storage(storage_context)
            else:
                logger.info("Creating new DGX index with LlamaParse...")
                ...
                if not documents:
                    return f"❌ No DGX documentation files found in {docs_path}"
            ...
            logger.info("Executing DGX query: %s", query)
            ...
            result += "📋 **Source:** DGX Administration Manual\n\n"
            result += "⚠️  **Note:** Please verify commands in your specific DGX environment before execution."

            return result

        except Exception as e:
            logger.error("Error in DGX documentation search: %s", str(e))
            return f"❌ Error in DGX analysis: {str(e)}\n\nPlease check your API keys and network connection."

    yield FunctionInfo.from_fn(_search_dgx_docs,
                               description="Analyze DGX node requirements and configuration for DGX cluster setup")


print("✅ DGX tool registered successfully")
